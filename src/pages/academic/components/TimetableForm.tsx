...
export function TimetableForm({ mode, timetableId, prefilledData, onSuccess, onCancel, lockContextFields }: TimetableFormProps) {
    ...
    // Fetch subject assignments when section changes
    useEffect(() => {
        if (formData.section) {
            fetchSubjectAssignments(formData.section);
        } else {
    ...
    const fetchSubjectAssignments = async (sectionId: number) => {
        try {
            setLoadingAssignments(true);
            const data = await subjectAssignmentApi.list({
                section: sectionId,
                page_size: 100,
                is_active: true
            });
            setSubjectAssignments(data.results);
        } catch (err) {
    ...
    const fetchClassrooms = async () => {
        try {
            const data = await classroomApi.list({ page_size: 100, is_active: true });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('Submitting timetable data:', formData); // ✅ Debug log
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
                    ...
                    <SelectContent>
                        {subjectAssignments.map((a) => (
                            <SelectItem key={a.id} value={a.id.toString()}>
                                {a.subject_details?.name || a.subject_name}
                                {a.teacher_details && ` - ${a.teacher_details.full_name}`}
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Select>
                {selectedAssignment && (
                    <p className="text-xs text-muted-foreground">
                        Teacher: {selectedAssignment.teacher_details?.full_name || 'Not assigned'}
                    </p>
                )}
            </div>
