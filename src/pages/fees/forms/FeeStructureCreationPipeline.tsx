...
export const FeeStructureCreationPipeline = ({ onSubmit, onCancel }: FeeStructureCreationPipelineProps) => {
  ...
  const [existingFeeGroups, setExistingFeeGroups] = useState<any[]>([]);
  const [existingFeeTypes, setExistingFeeTypes] = useState<any[]>([]);
  const [existingFeeMasters, setExistingFeeMasters] = useState<any[]>([]);
  const [loadingFeeGroups, setLoadingFeeGroups] = useState(false);
  const [loadingFeeTypes, setLoadingFeeTypes] = useState(false);
  const [loadingFeeMasters, setLoadingFeeMasters] = useState(false);
  ...
  const { data: academicYearsData } = useAcademicYears({ page_size: 100 });
  const { data: studentsData } = useStudents({ page_size: 200, is_active: true });
  ...
  // Fetch fee types when fee group is selected
  useEffect(() => {
    if (feeGroupMode === 'existing' && selectedFeeGroupId) {
      fetchFeeTypes(selectedFeeGroupId);
    } else {
      setExistingFeeTypes([]);
      setValue('existingFeeTypeId', null);
    }
  }, [feeGroupMode, selectedFeeGroupId]);
  ...
  const fetchFeeGroups = async () => {
    try {
      setLoadingFeeGroups(true);
      const data = await feeGroupsApi.list({
        page_size: 200,
        is_active: true
      });
      setExistingFeeGroups(data.results);
    } catch (err) {
      console.error('Failed to fetch fee groups:', err);
      setExistingFeeGroups([]);
    } finally {
      setLoadingFeeGroups(false);
    }
  };

  const fetchFeeTypes = async (feeGroupId: number) => {
    try {
      setLoadingFeeTypes(true);
      const data = await feeTypesApi.list({
        fee_group: feeGroupId,
        page_size: 200,
        is_active: true
      });
      setExistingFeeTypes(data.results);
    } catch (err) {
  ...
  const fetchFeeMasters = async () => {
    try {
      setLoadingFeeMasters(true);
      const data = await feeMastersApi.list({ page_size: 200, is_active: true });
      setExistingFeeMasters(data.results);
    } catch (err) {
      console.error('Failed to fetch fee masters:', err);
      setExistingFeeMasters([]);
    } finally {
      setLoadingFeeMasters(false);
    }
  };
  ...
  const steps = [
    ...
    { number: 3, title: 'Fee Master', icon: DollarSign, description: 'Select or create fee master' },
