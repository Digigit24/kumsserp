...
import { useHostels, useCreateHostel, useUpdateHostel, useDeleteHostel } from '../../hooks/useHostel';
...
const HostelsPage = () => {
  ...
  const { data, isLoading, error, refetch } = useHostels(filters);
  ...
  return (
    <div>
      <DataTable title="Hostels" description="Manage hostels" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Hostel" />
