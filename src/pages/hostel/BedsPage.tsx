...
import { useHostelBeds, useCreateHostelBed, useUpdateHostelBed, useDeleteHostelBed } from '../../hooks/useHostel';
...
const BedsPage = () => {
  ...
  const { data, isLoading, error, refetch } = useHostelBeds(filters);
  ...
  return (
    <div>
      <DataTable title="Hostel Beds" description="Manage hostel beds" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Bed" />
