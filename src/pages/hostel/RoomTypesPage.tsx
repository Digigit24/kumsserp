...
import { useRoomTypes, useCreateRoomType, useUpdateRoomType, useDeleteRoomType } from '../../hooks/useHostel';
...
const RoomTypesPage = () => {
  ...
  const { data, isLoading, error, refetch } = useRoomTypes(filters);
  ...
  return (
    <div>
      <DataTable title="Room Types" description="Manage room types" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Room Type" />
