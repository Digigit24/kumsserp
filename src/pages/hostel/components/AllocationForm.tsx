...
export const AllocationForm = ({ item, onSubmit, onCancel }: AllocationFormProps) => {
  ...
  const { data: studentsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(buildApiUrl('/api/v1/students/students/?page_size=1000'), {
        headers: {
          ...getDefaultHeaders(),
  ...
  const { data: roomsData } = useQuery({
    ...
    queryFn: async () => {
      const token = localStorage.getItem('kumss_auth_token');
      const response = await fetch(buildApiUrl(`/api/v1/hostel/rooms/?hostel=${hostel}&page_size=1000`), {
        headers: {
          ...getDefaultHeaders(),
          Authorization: `Token ${token}`,
        },
  ...
  // Fetch beds
  const { data: bedsData } = useHostelBeds(room ? { room, page_size: 1000 } : { page_size: 1000 });
