...
export const HostelForm = ({ item, onSubmit, onCancel }: HostelFormProps) => {
  ...
  const { data: collegesData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(buildApiUrl('/api/v1/core/colleges/?page_size=1000'), {
        headers: {
          ...getDefaultHeaders(),
  ...
  // Fetch users (for warden selection)
  const { data: usersData } = useQuery({
    queryKey: ['users-warden'],
    queryFn: async () => {
      const token = localStorage.getItem('kumss_auth_token');
      const response = await fetch(buildApiUrl('/api/v1/accounts/users/?page_size=1000'), {
        headers: {
          ...getDefaultHeaders(),
          Authorization: `Token ${token}`,
        },
  ...
  const wardenOptions = usersData?.results?.map((u: any) => ({
    value: u.id,
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <SearchableSelect
          ...
          value={college}
          onChange={(value) => setValue('college', value)}
          placeholder="Select college..."
          searchPlaceholder="Search colleges..."
        />
        {errors.college && <p className="text-sm text-destructive">{errors.college.message as string}</p>}
      </div>
      ...
      <div className="space-y-2">
        ...
        <SearchableSelect
          ...
          value={warden}
          onChange={(value) => setValue('warden', value)}
          placeholder="Select warden..."
          searchPlaceholder="Search users..."
        />
        {errors.warden && <p className="text-sm text-destructive">{errors.warden.message as string}</p>}
      </div>
