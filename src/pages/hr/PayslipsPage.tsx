...
import { usePayslips, useCreatePayslip, useUpdatePayslip, useDeletePayslip } from '../../hooks/useHR';
...
const PayslipsPage = () => {
  ...
  const { data, isLoading, error, refetch } = usePayslips(filters);
  ...
  return (
    <div>
      <DataTable title="Payslips" description="Manage payslips" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Payslip" />
