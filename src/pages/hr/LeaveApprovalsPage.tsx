...
import { useLeaveApprovals, useCreateLeaveApproval, useUpdateLeaveApproval, useDeleteLeaveApproval } from '../../hooks/useHR';
...
const LeaveApprovalsPage = () => {
  ...
  const { data, isLoading, error, refetch } = useLeaveApprovals(filters);
  ...
  return (
    <div>
      <DataTable title="Leave Approvals" description="Manage leave approvals" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Leave Approval" />
