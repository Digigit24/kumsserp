...
import { useLeaveApplications, useCreateLeaveApplication, useUpdateLeaveApplication, useDeleteLeaveApplication } from '../../hooks/useHR';
...
const LeaveApplicationsPage = () => {
  ...
  const { data, isLoading, error, refetch } = useLeaveApplications(filters);
  ...
  return (
    <div>
      <DataTable title="Leave Applications" description="Manage leave applications" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Leave Application" />
