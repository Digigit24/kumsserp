...
import { useSalaryComponents, useCreateSalaryComponent, useUpdateSalaryComponent, useDeleteSalaryComponent } from '../../hooks/useHR';
...
const SalaryComponentsPage = () => {
  ...
  const { data, isLoading, error, refetch } = useSalaryComponents(filters);
  ...
  return (
    <div>
      <DataTable title="Salary Components" description="Manage salary components" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Salary Component" />
