...
import { useLeaveBalances, useCreateLeaveBalance, useUpdateLeaveBalance, useDeleteLeaveBalance } from '../../hooks/useHR';
...
const LeaveBalancesPage = () => {
  ...
  const { data, isLoading, error, refetch } = useLeaveBalances(filters);
  ...
  return (
    <div>
      <DataTable title="Leave Balances" description="Manage leave balances" columns={columns} data={data || null} isLoading={isLoading} error={error?.message || null}
        filters={filters} onFiltersChange={setFilters} onRowClick={handleRowClick} onRefresh={refetch} onAdd={handleAddNew} addButtonLabel="Add Leave Balance" />
