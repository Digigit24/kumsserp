...
// Mock Data
const NOTICES = [
    { id: 1, title: 'Year-end Inventory Audit', time: '2 days ago', tag: 'Urgent', type: 'admin' },
    { id: 2, title: 'New Vendor Policy Update', time: '1 week ago', tag: 'Policy', type: 'procurement' },
    { id: 3, title: 'System Maintenance Scheduled', time: 'Today', tag: 'System', type: 'it' },
];

const MESSAGES = [
    { id: 1, sender: 'Principal Office', subject: 'Approval needed for Lab Equipment', time: '10 min ago' },
    { id: 2, sender: 'Science Dept (HOD)', subject: 'Clarification on chemical indent', time: '1 hour ago' },
    { id: 3, sender: 'Accounts Dept', subject: 'Vendor payment processed', time: 'Yesterday' },
];
            ...
            <CardContent>
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    <AnimatePresence mode="wait">
                            ...
                            <TabsContent value="notices" className="space-y-3 mt-0">
                                {NOTICES.map((note, index) => (
                                    <motion.div
                                        ...
                                        onClick={() => navigate(`/communication/notices/${note.id}`)}
                                    >
                                        <div className="flex gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-muted-foreground mt-1.5">{note.time}</p>
                                            </div>
                                        </div>
                                        <Badge variant={note.tag === 'Urgent' ? 'destructive' : 'outline'} className="rounded-md">{note.tag}</Badge>
                                    </motion.div>
                                ))}
                        ...
                        <motion.div
                            key={activeTab + "-content"} // Unique key to force re-render/animate
                            initial={{ opacity: 0, x: 10 }}
                            animate={{ opacity: 1, x: 0 }}
                            exit={{ opacity: 0, x: -10 }}
                            transition={{ duration: 0.2 }}
                                ...
                                {MESSAGES.map((msg, index) => (
                                    ...
                                    >
                                        <div className="flex gap-4">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-semibold leading-none group-hover:text-blue-600 transition-colors">{msg.sender}</h4>
                                                <p className="text-xs text-foreground mt-1.5 line-clamp-1">{msg.subject}</p>
